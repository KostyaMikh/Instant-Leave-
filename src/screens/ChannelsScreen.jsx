...
const FILTER = { ALL: 'all', CHANNELS: 'channels', GROUPS: 'groups' };
...
export default function ChannelsScreen({ user, onLogout }) {
  ...
  async function handleLeave() {
    ...
    try {
      ...
      setError(err.message || 'Failed to leave some chats');
    } finally {
      setLeaving(false);
    }
  }
      ...
      {loading ? (
        ...
        <div className="empty-state">
          ...
          <p className="empty-sub">
            {search ? 'Try a different search term' : 'You have no channels or groups to leave.'}
          </p>
          {!search && <button className="btn btn-ghost" onClick={loadDialogs}>Refresh</button>}
        </div>
      ) : (
        <>
